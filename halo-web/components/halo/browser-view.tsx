...
export function BrowserView({ agent }: { agent: Address }) {
  ...
  const [paused, setPaused] = useState(false), [selected, setSelected] = useState(""), [image, setImage] = useState<{ hash: string; url: string } | null>(null), [now, setNow] = useState(0), [fullscreen, setFullscreen] = useState(false);
  ...
  return <div className="browser-view" ref={container}>
    <div className="row between"><div className="row" style={{ gap: 8 }}><Monitor size={18} /><h3>{current?.frame.surface === "desktop" ? "Inside the agent’s desktop" : "Inside the agent’s browser"}</h3></div>
      <span className={`chip ${live && !paused ? "green live" : ""}`}>{live && !paused && <i aria-hidden="true" />}{paused ? "Playback paused" : live ? "Live" : latest ? "Last recorded view" : "Waiting for a browser"}</span></div>
    <div><div className="browser-chrome"><span className="dots" aria-hidden="true"><i /><i /><i /></span><span className="url">{current?.frame.siteOrigin ?? "Isolated browser session"}</span>
      <button type="button" className="pill icon sm" aria-label={fullscreen ? "Exit fullscreen viewer" : "Expand browser viewer"} onClick={() => { (fullscreen ? document.exitFullscreen?.() : container.current?.requestFullscreen?.())?.catch(() => setStream(s => ({ ...s, error: "Fullscreen is unavailable in this browser." }))); }}>{fullscreen ? <Minimize2 size={14} /> : <Maximize2 size={14} />}</button></div>
      <div className="browser-screen">
        {/* Verified blob URLs cannot go through next/image. */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        {shown ? <img src={shown} alt={`Operator ${current?.frame.surface === "desktop" ? "desktop" : "browser"}: ${current?.frame.activity}`} width={current?.frame.width} height={current?.frame.height} /> : <div className="browser-empty">{privacy ? <LockKeyhole size={28} /> : <Monitor size={30} />}<h4>{privacy ? "Private browser step" : current ? "Screen unavailable" : "A window into the work."}</h4><p>{privacy ? "Login, identity checks and credentials are never shown." : current?.frame.activity ?? "When an operator connects its browser, public research and social activity appear here."}</p></div>}
        {current?.frame.source === "development-capture" && <span className="chip browser-tag">Development capture</span>}
        {current?.frame.source === "local-browser-worker" && <span className="chip browser-tag">Local Linux {current.frame.surface === "desktop" ? "desktop" : "worker"}</span>}
        {current?.frame.sandbox === "container-only" && <span className="chip orange browser-tag" style={{ left: "auto", right: 10 }}>Container isolation only</span>}
      </div></div>
    <div className="browser-controls"><button type="button" className="pill icon sm" disabled={!latest} aria-label={paused ? "Resume latest screen" : "Pause browser playback"} onClick={() => { setSelected(current?.hash ?? ""); setPaused(v => !v); }}>{paused ? <Play size={14} /> : <Pause size={14} />}</button>
      <p>{current?.frame.activity ?? "Public browser activity appears automatically."}</p><time dateTime={current?.frame.timestamp}>{current ? new Date(current.frame.timestamp).toLocaleTimeString() : "—"}</time></div>
    ...
    <p className="dim">{current ? `${shown ? "Screen hash and signature verified" : "Report signature verified"} · Operator ${shortAddress(current.frame.operator)}.` : "Read-only viewer · no remote control of the agent."} A signature identifies the publisher; it cannot prove nobody else controlled the browser.</p>
    {current && <details className="disclosure"><summary>Browser record &amp; signature</summary><code>{current.hash}</code><code>{current.signature}</code></details>}
  </div>;
}
