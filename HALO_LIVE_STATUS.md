...
> **15 September 2026, evening — hosting checkpoint.** Code is on GitHub (`monarchoct/halo`, root CI). Inference: a Qwen3.5-4B llama.cpp lease runs on Akash (RTX 3090, ~$0.17/h) and produced the first real model-driven child launch (HERMES, under agent Lyra) on the local chain. Host: a Hostinger KVM 4 (Ubuntu 24.04) is bootstrapped with the reviewed runtime and browser images built on-host, PostgreSQL 17 with TLS, migrations (schema 4), role grants, the operations projection, the history journal schema and **three healthy IPFS peers**; `deploy/host/` now ships the three-peer compose, an HTTPS front for the Akash lease (`inference.<domain>`) and `set-domain.sh`. **Blocked on the owner:** test ETH for the three testnet wallets (all 0 ETH — the faucet needs a browser/wallet) and the public domain; the chain-facing services (API, operations, relays, operator, Caddy) start the moment `deploy/testnet/config.json` is deployed with `scripts/deploy-test-token.mjs` + `scripts/deploy-public.mjs`.
