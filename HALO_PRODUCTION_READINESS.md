...
- **First model-driven agent on rented GPU.** The pinned Qwen3.5-4B release runs on Akash (RTX 3090, provider pronto-ai, llama.cpp server with an OpenAI-compatible API, 32k context, key-protected). Agent `Lyra` (public-model mode) completed a cycle whose proposal came from that model: child coin HERMES launched, 5.4 s inference, transcript and release hashes on IPFS, receipt confirmed. Until the endpoint sits behind a real TLS hostname, a loopback proxy (`scripts/inference-proxy.mjs`) bridges it for the runtime.
