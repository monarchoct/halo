...
await assert.rejects(runSocialTask(f), /already-connected account binding/); assert.equal(f.page.submits, 0);
passed.push('Publishing without an already-connected binding is refused -- there is no self-service onboarding here');
f = fixture(); f.binding.profileUrl = f.binding.profileUrl.replace('halo_test', 'someone_else');
const loggedOut = await runSocialTask(f);
assert.equal(loggedOut.status, 'account-mismatch'); assert.equal(f.page.submits, 0); assert.equal(f.reports.at(-1).state, 'needs-account');
passed.push('A connected account whose visible identity no longer matches its configured profile is reported needs-account (mapped to credentials-expired by the caller), never as a signup opportunity');
